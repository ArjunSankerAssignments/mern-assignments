...
let Books = function (_id, cover, title, author, price, reviewer, rating, reviews) {
    this._id = _id;
    this.cover = cover;
    this.title = title;
    this.author = author;
    this.price = price;
    //this.rating = checkEmpty(rating);
    this.reviews = addFirstReview(reviewer, rating, reviews);
    this.details = undefined;
    this.delete = undefined;
};

let addFirstReview = function (reviewer, rating, review) {
    retVal = [];
    if(reviewer === undefined || rating === undefined || review === undefined)
        return retVal;
    retVal.push(new Review(reviewer, rating, review));
    return retVal;
}

Books.prototype.printBook = function () {
    let tableField = document.getElementById("booksTable");
    ...
    tdList[0].appendChild(coverImg);
    tdList[1].appendChild(document.createTextNode(this.title));
    tdList[2].appendChild(document.createTextNode(this.author));
    tdList[3].appendChild(document.createTextNode(this.price));

    detailsRef = document.createElement("a");
    detailsRef.appendChild(document.createTextNode("Details"));
    this.details = detailsRef;

    tdList[4].appendChild(this.details);

    tableField.appendChild(newRow);
    for (let df of tdList)
        newRow.appendChild(df);
};

Books.prototype.jumpToDetails = function () {
    localStorage.setItem("latestDetails", JSON.stringify(this));
...
let checkEmpty = function (checkObj) {
    let returnArray = [];
    if (checkObj) {
        returnArray.push(checkObj);
    }
    return returnArray;
};

let objToBooks = function(obj){
    let book = new Books;
...
function init() {
    ...
    if (data === null || data === undefined) {
        for (let book of books) {
            bookList.push(new Books(
                book["_id"], book["cover"], book["title"], book["author"], book["price"], "Anonymous", book["rating"], book["description"]
            ));
        }
    }else{
        ...
        for(let obj of objList){
            bookList.push(objToBooks(obj));
        }
        console.log(bookList);
    }
    
    if (localStorage.getItem("reviewFlag") !== null) {
        let bookId = localStorage.getItem("reviewFlag");
        for (book of bookList) {
            if (book["_id"] === bookId) {
                let newReview = JSON.parse(localStorage.getItem(bookId));
    ...
    if(localStorage.getItem("newBookFlag") != null){
        addedBook = JSON.parse(localStorage.getItem("newBook"));
        bookList.push(new Books(addedBook["_id"], addedBook["cover"], addedBook["title"], addedBook["author"], addedBook["price"]));
    }

    for (let book of bookList) {
        book.printBook();
        //console.log(book.price);
        book.details.onclick = function () { book.jumpToDetails(); };
    }
